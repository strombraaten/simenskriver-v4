...
export const siteConfig: SiteConfig = {
  // Site Information
  // [CONFIG:SITE_URL]
  site: "https://www.simenskriver.no/",
  // [CONFIG:SITE_TITLE]
  title: "Simen skriver",
  ...
  description:
    "Simen skriver om teknologisk tankemat og ymse ting han lærer seg. Sjeldent i tredje person.",
  // [CONFIG:SITE_AUTHOR]
  ...
  fonts: {
    ...
    families: {
      // [CONFIG:FONT_BODY]
      body: "Atkinson Hyperlegible Next", // Body text font family
      // [CONFIG:FONT_HEADING]
      heading: "Atkinson Hyperlegible Next", // Heading font family
      // [CONFIG:FONT_MONO]
  ...
  navigation: {
    ...
    social: [
      ...
      {
        title: "Linkedin",
        url: "https://www.linkedin.com/in/strombraaten/",
        icon: "linkedin",
      },
    ],
  },
...
function validateSiteConfig(config: SiteConfig): {
  ...
  if (config.theme === "custom") {
    if (
      !config.customThemeFile ||
      config.customThemeFile.trim() === ""
    ) {
      errors.push(
  ...
  if (
    !["local", "cdn"].includes(config.fonts.source)
  ) {
    errors.push(
  ...
  if (
    !config.fonts.families.body ||
    config.fonts.families.body.trim() === ""
  ) {
    errors.push(
  ...
  if (
    !config.fonts.families.mono ||
    config.fonts.families.mono.trim() === ""
  ) {
    errors.push(
  ...
  if (
    !["swap", "fallback", "optional"].includes(
  ...
  if (
    config.tableOfContents.depth < 2 ||
    config.tableOfContents.depth > 6
  ) {
    errors.push(
  ...
  if (
    ...
    !config.layout.contentWidth.match(
      /^\d+(\.\d+)?(rem|px|em)$/,
    )
  ) {
    errors.push(
  ...
  if (
    !["minimal", "traditional"].includes(
      config.navigation.style,
    )
  ) {
    errors.push(
  ...
  if (
    !validCoverImageOptions.includes(
      config.postOptions.showPostCardCoverImages,
    )
  ) {
    errors.push(
  ...
  if (
    !validAspectRatios.includes(
      config.postOptions.postCardAspectRatio,
    )
  ) {
    errors.push(
  ...
  if (
    config.postOptions.postCardAspectRatio === "custom"
  ) {
    if (
  ...
  if (
    !["above", "below", "none"].includes(
      config.homeOptions.blurb.placement,
    )
  ) {
    errors.push(
  ...
  if (
    !["latest", "featured"].includes(
      config.homeOptions.featuredPost.type,
    )
  ) {
    errors.push(
  ...
  if (
    !config.language ||
    !config.language.match(/^[a-z]{2}(-[A-Z]{2})?$/)
  ) {
    errors.push(
  ...
  if (
    typeof config.footer.showSocialIconsInFooter !==
    "boolean"
  ) {
    errors.push(
  ...
  if (config.profilePicture.enabled) {
    ...
    if (
      !config.profilePicture.alt ||
      config.profilePicture.alt.trim() === ""
    ) {
      errors.push(
    ...
    if (
      !["footer", "header"].includes(
        config.profilePicture.placement,
      )
    ) {
      errors.push(
    ...
    if (
      !["circle", "square", "none"].includes(
        config.profilePicture.style,
      )
    ) {
      errors.push(
    ...
    if (
      config.profilePicture.url &&
      config.profilePicture.url.trim() === ""
    ) {
      errors.push(
        "Profile picture URL cannot be empty if provided.",
      );
    }
  }
